...
def _parse_fk_payload_keys(
    model: Model, payload_dict: Dict[str, Any]
) -> Dict[str, Any]:
...
def _get_model_router(model: Model, name: str) -> Router:
    ...
    ModelPost = create_schema(
        model=model, name=f"CRUD{name.title()}Post", exclude=["id"]
    )
    ModelGet = create_schema(model=model, name=f"CRUD{name.title()}Get")

    @router.post(
        f"/",
        operation_id=f"create_instance_{name}",
        summary=f"Create a new {model.__name__} instance from the given payload.",
    )
    async def create_instance(request, payload: ModelPost):
        ...
        LOGGER.debug(
            "Creating %s instance from parsed_dict %s", model.__name__, parsed_dict
        )
    ...
    @router.get(
        f"/" + "{model_id}",
        response=ModelGet,
        operation_id=f"get_instance_{name}",
        summary=f"Return an {model.__name__} instance for the given id.",
    )
    ...
    @router.get(
        f"/",
        response=List[ModelGet],
        operation_id=f"list_instances_{name}",
        summary=f"Return a list of all {model.__name__} instances.",
    )
    ...
    @router.put(
        f"/" + "{model_id}",
        operation_id=f"update_instance_{name}",
        summary=f"Update the {model.__name__} instance with the payload for the given id.",
    )
    ...
    @router.patch(
        f"/" + "{model_id}",
        operation_id=f"patch_instance_{name}",
        summary=f"Patch the {model.__name__} instance with the payload for the given id.",
    )
    async def patch_instance(request, model_id: int, payload: ModelPost):
        """Update the model instance with the payload."""
        LOGGER.info("Patching model ID %s", model_id)
        instance = await sync_to_async(get_object_or_404)(model, id=model_id)
        parsed_dict = _parse_fk_payload_keys(model, payload.dict())
        for attr, value in parsed_dict.items():
            setattr(instance, attr, value)
    ...
    @router.delete(
        f"/" + "{model_id}",
        operation_id=f"delete_instance_{name}",
        summary=f"Delete the {model.__name__} instance for the given id.",
    )
...
def add_model_crud_route(api, name, model, auth=None):
    ...
    LOGGER.info(
        "Adding model CRUD route for model %s under name %s", model.__name__, name
    )
